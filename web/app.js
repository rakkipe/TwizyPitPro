...
let writeSelection=new Set(), vehicleMessage='';
...
function render(){
  ...
  if(!draft||draftIdentity!==c.fingerprint){draft={...Object.fromEntries(state.parameters.map(p=>[p.key,p.default])),...state.current};draftIdentity=c.fingerprint;writeSelection.clear();renderEditor();}
...
function renderEditor(){const groups=[...new Set(state.parameters.map(p=>p.group))];$('group-filter').innerHTML='<option value="all">Alle instellingen</option>'+groups.map(g=>`<option value="${esc(g)}">${esc(g)}</option>`).join('');$('parameter-groups').innerHTML=groups.map(g=>`<article class="panel parameter-group" data-group="${esc(g)}"><div class="group-heading"><h3>${esc(g)}</h3><span>${state.parameters.filter(p=>p.group===g).length} INSTELLINGEN</span></div>${state.parameters.filter(p=>p.group===g).map(p=>`<div class="parameter-row" data-key="${p.key}"><div class="parameter-header"><label for="num-${p.key}">${esc(p.name)}</label><div class="number-field"><input type="number" id="num-${p.key}" data-key="${p.key}" min="${p.min}" max="${p.max}" step="1" value="${draft[p.key]}" aria-label="${esc(p.name)}"><span>${esc(p.unit)}</span></div></div><p>${esc(p.note)}</p><div class="range-line"><span>${p.min}</span><input type="range" id="range-${p.key}" data-key="${p.key}" min="${p.min}" max="${p.max}" step="1" value="${draft[p.key]}" aria-label="${esc(p.name)} schuifregelaar"><span>${p.max}</span></div></div>`).join('')}</article>`).join('');
  $('parameter-groups').querySelectorAll('input').forEach(input=>input.addEventListener('input',()=>{const key=input.dataset.key;draft[key]=input.value===''?NaN:Number(input.value);const other=$(input.type==='range'?'num-'+key:'range-'+key);other.value=input.value;currentPlan=null;if(state.mode!=='demo')selectForWrite(key,true);renderTuneSummary();}));filterParameters();renderTuneSummary();}
...
const androidButton=document.createElement('button');androidButton.className='button small';androidButton.textContent='Android APK ↓';androidButton.onclick=()=>downloadAPI('/api/download/android','TwizyPitPro-0.3.1.apk').catch(e=>toast(e.message,true));$('phone-status').parentElement.appendChild(androidButton);
const vehiclePanel=document.createElement('article');
vehiclePanel.className='panel section-gap vehicle-write-panel';
vehiclePanel.innerHTML=`<div class="eyebrow">VLINKER FS / VOERTUIG</div><h3>Lezen, vergelijken en schrijven</h3><p class="muted">Lees de 75 echte instellingen via tijdelijke level-4-toegang. Kies daarna welke instellingen je wilt aanpassen en controleer het schrijfplan. De app vraagt geen wachtwoord.</p><div class="scenario-controls"><button class="button" id="snapshot-vehicle">75 instellingen uitlezen</button><button class="button" id="close-access" hidden>Toegangssessie afsluiten</button></div><details class="write-selection"><summary id="selection-summary">Te schrijven instellingen kiezen</summary><p class="helper">Een bewerkt veld wordt geselecteerd. Snelheid, koppel, stroom en vermogen vormen samen één set. Controleer bij die set alle vijf doelen. Overige registers blijven op hun uitgelezen waarde.</p><div class="scenario-controls"><button class="button small" id="select-all-writes">Alle 43 kiezen</button><button class="button small" id="select-no-writes">Selectie wissen</button></div><div id="write-fields"></div></details><label><input type="checkbox" id="faults-resolved"> Gemelde voertuigstoringen (ook SERV) zijn opgelost</label><label><input type="checkbox" id="stock-drivetrain"> Originele motor en reductiekast voor dit Twizy-model bevestigd</label><label><input type="checkbox" id="brake-hardware"> Aangepaste remlichthardware aanwezig (alleen nodig bij gewijzigde remlichtdrempels)</label><div class="scenario-controls"><button class="button primary" id="prepare-vehicle">Schrijfplan voorbereiden</button><button class="button" id="restore-vehicle" hidden>Herstelplan bekijken</button><button class="button" id="verify-vehicle" hidden>Na contactcyclus controleren</button></div><p id="vehicle-write-status" class="helper" role="status"></p><p id="vehicle-last-result" class="helper" role="status"></p>`;
...
function selectForWrite(key,on){
  const groups=[['speed','torque','current','power_low','power_high'],['brakelight_on','brakelight_off']];
  const keys=groups.find(g=>g.includes(key))||[key];
  keys.forEach(k=>on?writeSelection.add(k):writeSelection.delete(k));renderVehicleWriter();
}
async function vehicleApi(path,body){
  vehicleMessage='Bezig met voertuigcontrole…';
  try{const result=await api(path,body);vehicleMessage='Actie voltooid. '+(path.endsWith('snapshot')?'75 instellingen opgeslagen; level-4-toegang afgesloten.':'');return result;}
  catch(error){vehicleMessage=error.message;throw error;}
  finally{await refresh();}
}
action('select-all-writes',async()=>{writeSelection=new Set(state.parameters.map(p=>p.key));renderVehicleWriter();});
action('select-no-writes',async()=>{writeSelection.clear();renderVehicleWriter();});
action('snapshot-vehicle',async()=>{await vehicleApi('/api/vehicle/snapshot',{});setPage('diagnostics');toast('75 echte instellingen gelezen en opgeslagen.');});
action('close-access',async()=>{await vehicleApi('/api/vehicle/close-access',{});toast('Toegang afgesloten.');});
function renderVehicleWriter(){
  const pending=state.vehicle_transaction, access=state.vehicle_access;
  const fields=$('write-fields');
  if(fields.dataset.model!==state.compatibility.model){
    fields.dataset.model=state.compatibility.model;
    fields.innerHTML=state.parameters.map(p=>`<label><input type="checkbox" data-write-key="${p.key}"> ${esc(p.name)} <span data-write-value="${p.key}"></span></label>`).join('');
    fields.querySelectorAll('input').forEach(el=>el.addEventListener('change',()=>selectForWrite(el.dataset.writeKey,el.checked)));
  }
  fields.querySelectorAll('input').forEach(el=>el.checked=writeSelection.has(el.dataset.writeKey));
  fields.querySelectorAll('[data-write-value]').forEach(el=>{const p=state.parameters.find(p=>p.key===el.dataset.writeValue);el.textContent=`${format(draft[p.key])} ${p.unit}`;});
  $('selection-summary').textContent=`${writeSelection.size} ${writeSelection.size===1?'instelling':'instellingen'} geselecteerd voor schrijven`;
  $('vehicle-last-result').textContent=vehicleMessage;
  const eligible=bootstrap.local&&state.connected&&state.limits.live_write&&!state.busy&&!state.session;
  $('prepare-vehicle').disabled=!eligible||!!pending||!!access||!writeSelection.size;
  $('snapshot-vehicle').disabled=!eligible||!!pending||!!access;
  $('close-access').hidden=!access||!!pending;$('close-access').disabled=!eligible||access?.phase==='corrupt';
...
function showVehicleReview(plan,restore=false){
  ...
  $('vehicle-review').innerHTML=`<p class="helper">${esc(plan.identity.software)} · ${plan.changes.length} ${plan.changes.length===1?'gewijzigd register':'gewijzigde registers'}. ${restore?'Herstel gebruikt de originele backup van deze controller.':'Plan maximaal 120 seconden geldig; beginwaarden worden vóór schrijven opnieuw gecontroleerd.'}</p><div class="table-scroll"><table><thead><tr><th>REGISTER</th><th>UITGELEZEN</th><th>${restore?'ORIGINEEL':'DOEL'}</th></tr></thead><tbody>${plan.changes.map(r=>`<tr><td>${esc(r.address)}</td><td>${esc(r.before)}</td><td>${esc(r.raw)}</td></tr>`).join('')}</tbody></table></div><p class="helper">Level 4 → configuratiemodus → schrijven en teruglezen → controle na contactcyclus. Bij een fout stopt de procedure en blijft een hersteljournaal bewaard.</p>`;
  $('confirm-vehicle').textContent=restore?'Originele waarden herstellen':'Dit plan naar de Twizy schrijven';
  vehicleDialog.showModal();
}
action('prepare-vehicle',async()=>{const plan=await vehicleApi('/api/vehicle/prepare',{values:draft,selected_keys:[...writeSelection],stock_drivetrain:$('stock-drivetrain').checked,brake_hardware:$('brake-hardware').checked,faults_resolved:$('faults-resolved').checked});await refresh();showVehicleReview(plan);});
action('restore-vehicle',async()=>{const plan=await vehicleApi('/api/vehicle/restore-plan',{transaction_id:state.vehicle_transaction.id});await refresh();showVehicleReview(plan,true);});
action('confirm-vehicle',async()=>{const review=vehicleReview;vehicleReview=null;vehicleDialog.close();if(!review)return;try{await vehicleApi(review.restore?'/api/vehicle/restore':'/api/vehicle/apply',review.restore?{transaction_id:review.plan.id}:{plan_id:review.plan.id,review_hash:review.plan.review_hash});toast('Teruggelezen waarden kloppen. De contactcyclus en hercontrole staan nog open.');}finally{await refresh();}});
action('verify-vehicle',async()=>{await vehicleApi('/api/vehicle/verify-cycle',{transaction_id:state.vehicle_transaction.id});await refresh();toast('Alle waarden na de waargenomen contactcyclus gecontroleerd.');});
