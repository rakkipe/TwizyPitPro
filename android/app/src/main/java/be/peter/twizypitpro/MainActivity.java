...
public final class MainActivity extends Activity implements LocationListener {
    ...
    private String displayModel(){return mode.equals("demo")?selectedModel():Model.modelOf(identity);}
    private String displayFirmware(){return mode.equals("demo")?selectedSoftware():identity.optString("software","Onbekend");}
    ...
    private void tuning(){
        if(mode.equals("usb")&&Model.modelOf(identity).equals("?")){title("ONBEKENDE CONTROLLER","Eerst identificeren.","Geen passende modelreferentie; alleen diagnose beschikbaar.");return;}
        title("SETUP STUDIO / ONTWERP","Elke instelling telt.","43 ontwerpvelden · Twizy "+selectedModel()+" / "+selectedSoftware());
        ...
        addButton(info,"Registerdoelen & scanvergelijking",false,()->registerPlan());
    ...
    private void renderParameters(LinearLayout groups,String query){groups.removeAllViews();JSONArray params=model.parameters(selectedModel());String last="";query=query.toLowerCase(Locale.ROOT);
    ...
    private String selectedModel(){return mode.equals("usb")?Model.modelOf(identity):config.optString("model");}
    private String selectedSoftware(){return mode.equals("usb")?identity.optString("software"):config.optString("software");}
    private void compareDraft(){
        if(mode.equals("usb")){registerPlan();return;}
        model.validate(draft,selectedModel());JSONObject proposed=Model.copy(draft);JSONObject before=config.optJSONObject("values");StringBuilder diff=new StringBuilder();JSONArray params=model.parameters(selectedModel());int count=0;
    ...
    private void registerPlan(){
        String type=selectedModel();model.validate(draft,type);
        JSONObject matched=null;
        if(mode.equals("usb")&&scan!=null&&scan.optJSONObject("identity")!=null){
            JSONObject id=scan.optJSONObject("identity");
            if(Model.modelOf(id).equals(type)&&id.optString("software").equals(selectedSoftware())&&id.toString().equals(identity.toString()))matched=scan;
        }
        JSONArray rows=Tuning.compare(model,draft,type,matched);int read=0;StringBuilder text=new StringBuilder();
        for(int i=0;i<rows.length();i++){JSONObject r=rows.optJSONObject(i);if(r.optBoolean("verified_width"))read++;text.append(r.optString("address")).append("  ").append(r.isNull("before")?"?":r.opt("before")).append(" → ").append(r.isNull("raw")?"eerst flags uitlezen":r.opt("raw")).append("\n");}
        JSONObject report=new JSONObject();put(report,"at",Model.now());put(report,"model",type);put(report,"software",selectedSoftware());put(report,"identity",matched==null?JSONObject.NULL:Model.copy(identity));put(report,"values",Model.copy(draft));put(report,"register_targets",rows);put(report,"executable",false);
        put(report,"scope","Registerdoelen voor standaard aandrijflijn. Geen uitvoerbare schrijfvolgorde of hardwarekwalificatie. Control flags worden uitsluitend uit de scan behouden. 4641:01 is een afzonderlijke write-only kaartcommit.");
        new AlertDialog.Builder(this).setTitle("43 velden → "+rows.length()+" registers").setMessage(read+" beginwaarden uit een passende USB-scan.\n\n"+text+"\n"+report.optString("scope")).setNegativeButton("Terug",null).setPositiveButton("Exporteer…",(d,w)->export(report.toString(),"twizy-registerdoelen.json","application/json")).show();
    }
    ...
    private String demoBlocker(){if(!mode.equals("demo"))return "Live schrijftoegang ontbreekt. Alleen een demo-afstelling kan worden toegepast.";if(demoRunning||recording)return "Stop de demoronde en opname vóór toepassing.";if(config.has("pending"))return "Rond eerst de vorige demo-contactcyclus af.";if(!Model.known(selectedModel(),selectedSoftware(),Model.revision(selectedSoftware())))return "Onbekende of vergrendelde firmwarecombinatie.";if(!config.optString("fault").matches("none|writefail"))return "Los het gesimuleerde foutscenario eerst op.";return "";}
    ...
    private void saveProfile(){model.validate(draft,selectedModel());JSONObject values=Model.copy(draft);EditText name=input("Naam","Circuit · setup "+(config.optJSONArray("profiles").length()+1));new AlertDialog.Builder(this).setTitle("Profiel bewaren").setView(name).setNegativeButton("Annuleer",null).setPositiveButton("Goedkeuren…",(d,w)->{
        try{String label=name.getText().toString().trim();if(label.isEmpty()||label.length()>80)throw new IllegalArgumentException("Gebruik een profielnaam van 1–80 tekens.");JSONObject p=new JSONObject();put(p,"format","TwizyPitPro-profile-v1");put(p,"name",label);put(p,"model",selectedModel());put(p,"software",selectedSoftware());put(p,"revision",Model.revision(selectedSoftware()));put(p,"values",values);put(p,"at",Model.now());JSONObject next=Model.copy(config);JSONArray profiles=next.optJSONArray("profiles");if(profiles.length()>=100)throw new IllegalArgumentException("Maximaal 100 profielen. Exporteer en verwijder eerst een oud profiel.");profiles.put(p);approve("Profiel opslaan: "+label,next,()->showPage(4));}catch(Exception e){error(e);}
    }).show();}
    private void profiles(){
        ...
        LinearLayout actions=card("Profielen uitwisselen","Import controleert model, firmware, revisie en alle waarden.");addButton(actions,"Ontwerp importeren uit JSON",false,()->{Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT).setType("application/json").addCategory(Intent.CATEGORY_OPENABLE);startActivityForResult(i,42);});addButton(actions,"Huidig ontwerp exporteren",false,()->{model.validate(draft,selectedModel());JSONObject p=new JSONObject();put(p,"format","TwizyPitPro-profile-v1");put(p,"name","Android-ontwerp");put(p,"model",selectedModel());put(p,"software",selectedSoftware());put(p,"revision",Model.revision(selectedSoftware()));put(p,"values",Model.copy(draft));export(p.toString(),"twizy-ontwerp.json","application/json");});
    ...
    private void checkProfile(JSONObject p){if(!p.optString("format").equals("TwizyPitPro-profile-v1")||!p.optString("model").equals(selectedModel())||!p.optString("software").equals(selectedSoftware())||p.optLong("revision",-2)!=Model.revision(selectedSoftware()))throw new IllegalArgumentException("Model, firmware of dictionaryrevisie wijkt af van het gekozen ontwerpmodel.");model.validate(p.optJSONObject("values"),selectedModel());}
    ...
    private void stopRecording(){if(samples.length()==0)throw new IllegalStateException("Nog geen metingen om te bewaren.");recording=false;JSONObject session=new JSONObject();put(session,"name",recordName);put(session,"mode",recordMode);put(session,"at",Model.now());put(session,"samples",samples);put(session,"laps",laps);put(session,"identity",Model.copy(identity));put(session,"model",selectedModel());put(session,"software",selectedSoftware());put(session,"values",Model.copy(config.optJSONObject("values")));JSONObject next=Model.copy(config);next.optJSONArray("sessions").put(session);approve("Sessie bewaren: "+recordName,next,()->{samples=new JSONArray();laps=new JSONArray();showPage(3);});}
    ...
    private void garage(){
        title("GARAGE / EIGENAAR","Jouw pit. Jouw toestemming.","Android 0.3.1 · lokaal, ondertekend en zonder automatische updates.");
        ...
        LinearLayout simulation=card("Simulator","Model- en firmwarewissels passen alleen de lokale demo aan.");addButton(simulation,"Model: Twizy "+selectedModel(),false,()->choose("Demomodel",new String[]{"Twizy 45","Twizy 80"},n->changeScenario(n==0?"45":"80",selectedSoftware())));addButton(simulation,"Firmware: "+selectedSoftware(),false,()->choose("Firmware",new String[]{"0712.0001","0712.0002","0712.0003","9999.9999"},n->changeScenario(selectedModel(),new String[]{"0712.0001","0712.0002","0712.0003","9999.9999"}[n])));
    ...
    private JSONObject demoIdentity(){JSONObject id=new JSONObject();put(id,"product",selectedModel().equals("80")?0x0712302d:0x0712301b);put(id,"vendor",30);put(id,"revision",Model.revision(selectedSoftware()));put(id,"software",selectedSoftware());put(id,"hardware","DEMO-HW");put(id,"source","simulation");return id;}
    private void tick(){
